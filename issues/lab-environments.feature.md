...
**The identity follows from what the lab is for.** A build slot takes a dedicated service user, so that runs are isolated from each other and from anyone's account. A development lab may instead run as the developer's own user, which is what gives it their access — writing the bare repositories they can write, without a second permission scheme to maintain. The blast radius then stops being about privilege and becomes exactly what the unit binds read-write, which is the intended trade but makes the bind list the security decision. Credential directories stay out of it: an agent holding `~/.ssh`, `~/.gnupg` or a token store carries an identity no namespace constrains.

The masking has to be a whitelist for this to hold. Nineteen setuid binaries, two of them wanted; `passwd`, `chsh`, `chfn` and `gpasswd` act on the running user's own account, and closing `sudo` while leaving those open moves the problem rather than solving it.

**Abstract unix sockets belong to the network namespace.** No amount of `ProtectHome=` or `PrivateTmp=` conceals them, because they are not filesystem objects. A lab sharing the host's network namespace saw six, the X server's `@/tmp/.X11-unix` among them, and saw none under `PrivateNetwork=yes`. A lab running as the developer's own user is precisely the case where this matters, since anything authenticating by UID accepts it. Labs that need outward access therefore take a namespace of their own with controlled egress rather than the host's.
