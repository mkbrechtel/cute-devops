...
What "deploy" means is the target's business: a `post-receive` hook that checks out and restarts a service, a forge pipeline, a PaaS build, a VM pull. The worktree-side workflow doesn't care which.
...
```
   idea ──► EnterWorktree / git worktree add …
                       │
                       ▼
        /work/<project>/<category>/<branch>
                       │
                       ▼
              MR / merge into main
                       │
...
- **Same primitive.** Worktrees share refs with the bare repo (no push). The bare repo pushes to deploy remotes (no special protocol). Every transfer is `git`.
