...
- **Full user session environment**: a shell spawned by a plain system unit is not a login session — no PAM run, no `/run/user/<uid>`, no systemd user instance — which breaks rootless podman (cgroup delegation via `user@<uid>.service`, runtime state in `XDG_RUNTIME_DIR`). The template unit sets `PAMName=` against a role-shipped `/etc/pam.d/ttyd` (`pam_systemd` + `pam_limits`), so each instance is a real logind session and `XDG_RUNTIME_DIR` is provided automatically. (`Environment=XDG_RUNTIME_DIR=/run/user/%U` does not work: in system units `%U` is the manager's uid, not the `User=` uid.) Additionally the role enables lingering for each managed user (`loginctl enable-linger`), so user containers and `systemctl --user` services start at boot and survive `ttyd@<user>` stops — they live in the user's own `user.slice`, not under the ttyd unit.
