...
- **Full user session environment**: a shell spawned by a system unit is not a login session — no PAM run, no `/run/user/<uid>`, no systemd user instance — which breaks rootless podman (cgroup delegation via `user@<uid>.service`, runtime state in `XDG_RUNTIME_DIR`). The role enables lingering for each managed user (`loginctl enable-linger`) and sets `Environment=XDG_RUNTIME_DIR=/run/user/%U` and `Environment=DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/%U/bus` in the template unit, so rootless podman and `systemctl --user` work inside the terminal. User containers land in the user's own `user.slice`, so restarting `ttyd@<user>` does not kill them.
